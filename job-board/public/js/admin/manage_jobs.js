document.addEventListener('DOMContentLoaded', () => {
    ...
    const filterTable = () => {
        ...
        tableRows.forEach(row => {
            ...
            const title = cells[2].textContent.toLowerCase().trim();  
            const location = cells[3].textContent.toLowerCase().trim();  
            const jobType = cells[4].textContent.toLowerCase().trim(); 
            const jobStatus = cells[5].querySelector('.badge')
                ? cells[5].querySelector('.badge').textContent.toLowerCase().trim()  
